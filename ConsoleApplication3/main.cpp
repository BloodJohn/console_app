...
int main () //основная функция, которая вызывается при запуске исполняемого файла
{//начало тела основной функции
    //объявление переменной c типом данных, структура "Record"
    Record item;

    //выделение памяти для массива символов, указателю присваевается адрес начала выделенной области памяти
    ...
    item.age = 33;
    //необходимо, чтобы все переменные были определены (это именно определение - отличается от объявления тем, что в этот момент под объект выделяется память)
